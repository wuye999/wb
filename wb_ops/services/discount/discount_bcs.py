# -*- coding: utf-8 -*-
"""
wb_ops 折扣改价（旧版 BCS 方式，原 discount.py / wb_bcs_discount.py）

[旧版/按需保留] 把所有店铺（BCS 自动获取）中「折扣 > 阈值」的在架商品改为目标折扣（默认 >50% → 50%）。
走 BCS 云端 API（shopKeeper/price/batch），需携带价格，速度较慢。
默认不启用，除非用户主动通过 wb.py discount-bcs 调用。
"""
import csv
import os
import time
...
def write_csv(rows):
    ...
    path = os.path.join(config.LOG_DIR, f"折扣修改_bcs_{ts}.csv")
