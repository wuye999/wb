...
def process_shop(shop, root_version, args, rows, cn_map, resolver=None):
    ...
    for q in questions:
        ...
        cn = (cn_map.get(vc, {}).get("cn", "") if vc else "") or (resolver(vc) if resolver and vc else "")
...
def run(args):
    ...
    cn_map = {}
    resolver, _ = MappingRepository.build_vc_resolver()
    try:
        cn_map = MappingRepository.load_mapping_state()[0]
    except Exception:
        pass
    ...
    for shop in shops:
        try:
            st = process_shop(shop, root_version, args, rows, cn_map, resolver=resolver)
