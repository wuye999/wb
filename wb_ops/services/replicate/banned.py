...
def _card_cn(vc):
    """（可选）映射表中文名：走 MappingRepository 反查解析器，不直接依赖 catalog.mapping。"""
    global _vc_resolver
    if not vc:
        return ""
    try:
        if _vc_resolver is None:
            from wb_ops.storage.mapping_repo import MappingRepository
            _vc_resolver, _ = MappingRepository.build_vc_resolver()
        return _vc_resolver(vc, "")
    except Exception:
        return ""
