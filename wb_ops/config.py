...
WB_TEMPLATE_XLSX = os.path.join(DATA_DIR, "16.08.2026 13.16的价格与折扣更新模板.xlsx")

STATUS_JSON = os.path.join(STATE_DIR, "多店铺同步状态.json")
AUTO_ADD_JSON = os.path.join(STATE_DIR, "多店铺自动新增清单.json")

# 一次性审核文件（用后归档，merge 已把状态固化进映射表，缺它们不影响）
RESULT_JSON = os.path.join(DATA_DIR, "导出json结果.json")
UNMAPPED_JSON = os.path.join(DATA_DIR, "33核对.json")
REVIEW_JSON = os.path.join(DATA_DIR, "多店铺审核.json")
UNIFIED_JSON = os.path.join(DATA_DIR, "统一审核.json")
