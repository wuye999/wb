...
| `import-shelve` | 他人映射表导入上架：他人有我方无的商品（按 WB原始nmId 匹配）上架到我的店铺；新 vc 我方前缀优先（中文名命中商品价格表前缀码），价格 = floor(双倍售价)；**支持 `ozon-card-` 尾段格式**（他人 vc 为 `BCS-{前缀}-ozon-card-{WB商品码}` 时，新 vc 保留该段：`BCS-{我方前缀}-ozon-card-{WB商品码}`，拉取数据仍用 WB商品码列）；缺失 WB商品码列仍跳过；**启动默认不自动同步（用本地快照判断，可能滞后），加 `--sync` 才先同步全部店铺（约 2 分钟）** | `<他人映射表.xlsx>` + `--cn` / `--shops` / `--limit` / `--apply` / `--sync` / `--no-verify` / `--interval S` / `--cn-stock "中文名:库存,..."` |
