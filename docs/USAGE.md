...
- 马帮库存登记：`python wb.py mabang-stock-register --apply` 全量重建「马帮库存登记表」（马帮全部库存SKU 的库存总量/状态/图片）；每日新订单量列（时间段管理）：`python wb.py mabang-stock-daily --begin 2026-09-08 --apply`（删除 begin 前的旧日期列、补建区间内缺失列；填充=有单写数量、**无单留空**、清旧 0 值；⚠ `mabang-stock-register` 全量重建会清空各日列，重建后需重跑 `mabang-stock-daily --begin 2026-09-05` 回填）。
