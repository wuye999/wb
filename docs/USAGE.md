...
- 马帮库存登记：`python wb.py mabang-stock-register --url "<表格地址>" --apply` 全量重建「马帮库存登记表」（马帮全部库存SKU 的库存总量/状态/图片）；每日新订单量列：`python wb.py mabang-stock-daily --url "<表格地址>" --date 2026-09-10 --apply`（无订单填 0；⚠ `mabang-stock-register` 全量重建会清空各日列，重建后需重跑 `mabang-stock-daily --begin 2026-09-05` 回填）。
